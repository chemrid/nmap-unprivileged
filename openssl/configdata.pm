...
our %config = (
    ...
    "options" => "--prefix=/src/openssl-build --openssldir=/src/openssl-build/ssl --libdir=lib no-acvp-tests no-asan no-brotli no-brotli-dynamic no-buildtest-c++ no-crypto-mdebug no-crypto-mdebug-backtrace no-demos no-devcryptoeng no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fips no-fips-post no-fips-securitychecks no-fuzz-afl no-fuzz-libfuzzer no-h3demo no-jitter no-ktls no-loadereng no-md2 no-msan no-pie no-rc5 no-sctp no-shared no-ssl3 no-ssl3-method no-tests no-tfo no-trace no-ubsan no-ui-console no-unit-test no-uplink no-weak-ssl-ciphers no-winstore no-zlib no-zlib-dynamic no-zstd no-zstd-dynamic",
    "patch" => "1",
...
unless (caller) {
    ...
    use lib '/src/openssl/util/perl';
    use OpenSSL::fallback '/src/openssl/external/perl/MODULES.txt';
    ...
    if (scalar @ARGV == 0) {
        ...
        use lib '.';
        use lib '/src/openssl/Configurations';
        use gentemplate;
